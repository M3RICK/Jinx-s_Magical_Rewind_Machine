...
def build_lane_prompt(stats, mode="coach"):
    ...
    primary_role = stats.get("primary_role")
    ...
    role_text = ROLE_DISPLAY_NAMES.get(primary_role, "")
    role_context = f" as {role_text}" if role_text else ""

    if mode == "roast":
        prompt = f"""Roast {zone_name} performance{role_context} (2-3 sentences, brutal):
Matches: {matches} | Deaths: {deaths} | CS@10: {avg_cs} | Gold@10: {avg_gold_diff:+.0f}
2nd person, savage. IMPORTANT: Player role is {role_text if role_text else "unknown"}."""
    else:  # coach mode
        prompt = f"""Coach {zone_name} performance{role_context} (2-3 sentences):
Matches: {matches} | Deaths: {deaths} | CS@10: {avg_cs} | Gold@10: {avg_gold_diff:+.0f}
Wave management and trading tips. 2nd person. IMPORTANT: Player role is {role_text if role_text else "unknown"}."""
    return prompt
...
def build_region_prompt(stats, mode="coach"):
    ...
    role_text = ROLE_DISPLAY_NAMES.get(primary_role, "")
    role_context = f" ({role_text})" if role_text else ""

    if mode == "roast":
        prompt = f"""Roast {zone_name} gameplay{role_context} (2-3 sentences, savage):
Deaths: {deaths} | Time spent: {time_spent}%
2nd person, brutal jokes."""
    else:  # coach mode
        prompt = f"""Coach {zone_name} gameplay{role_context} (2-3 sentences):
Deaths: {deaths} | Time spent: {time_spent}%
