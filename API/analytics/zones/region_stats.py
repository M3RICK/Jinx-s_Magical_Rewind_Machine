...
def find_primary_role_in_region(matches, region_name):
    role_counts = {}
...
def extract_lane_performance(matches, region_name):
    ...
    return {
        ...
        "avg_gold_diff_at_10": round(
            sum(gold_diff_at_10_list) / len(gold_diff_at_10_list), 1
        ),
...
def extract_region_zone_stats(matches, zone_id, zone_config):
    ...
    primary_role = find_primary_role_in_region(matches, region_name)
    ...
    return {
        ...
        "avg_deaths_per_match": round(deaths_in_region / total_matches, 2)
        if total_matches > 0
        else 0,
        "lane_performance": lane_performance,
        "primary_role": primary_role,  # Add primary role to help AI identify Support vs ADC
    }
