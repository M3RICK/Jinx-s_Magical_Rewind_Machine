...
from API.utils.encryption import PUUIDEncryption
...
async def validate_riot_account(game_name, tag_line, region_config):
    print(f"\n[INFO] Looking up Riot account: {game_name}#{tag_line}...")

    try:
        async with Core() as core:
            riot_api = RiotAccountAPI(core)
            puuid = await riot_api.get_puuid(game_name, tag_line, region=region_config["region"])
...
def check_or_create_player(puuid, game_name, tag_line, region):
    print(f"\n[INFO] Checking player database...")

    try:
        encryption = PUUIDEncryption()
        dynamodb = get_dynamodb_resources()
        table = dynamodb.Table('Players')

        puuid_hash = encryption.hash_puuid(puuid)

        response = table.get_item(Key={'puuid_hash': puuid_hash})

        if 'Item' in response:
            print(f"[SUCCESS] Welcome back! Found existing player record.")
            table.update_item(
                Key={'puuid_hash': puuid_hash},
            ...
            encrypted_puuid = encryption.encrypt_puuid(puuid)
            player_uuid = str(uuid.uuid4())
            player_data = {
                'puuid_hash': puuid_hash,
...
async def run_chat(session_token=None):
    print("\n" + "=" * 50)
    ...
    puuid = await validate_riot_account(game_name, tag_line, region_config)
    if not puuid:
        print("\n[ERROR] Cannot proceed without valid Riot account.")
        return

    player_data = check_or_create_player(puuid, game_name, tag_line, region_config["region"])
    if not player_data:
